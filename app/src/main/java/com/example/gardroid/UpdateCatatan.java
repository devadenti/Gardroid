...
public class UpdateCatatan extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        cursor = db.rawQuery("SELECT * FROM catatan_tanaman WHERE jenis = '" +
                getIntent().getStringExtra("jenis") + "'",null);
        cursor.moveToFirst();
        ...
        ton1.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View arg0) {
                ...
                db.execSQL("update catatan_tanaman set jenis='"+
                        text2.getText().toString() +"', tgl='" +
                        text3.getText().toString()+"', hari_ke='"+
                        text4.getText().toString() +"', cuaca='" +
                        text5.getText().toString() +"', suhu='" +
                        text6.getText().toString() +"', kondisi='" +
                        text7.getText().toString() + "' where no='" +
                        text1.getText().toString()+"'");
                Toast.makeText(getApplicationContext(), "Berhasil", Toast.LENGTH_LONG).show();
                FarmingNotesActivity.ma.RefreshList();
                finish();
            }
        });
